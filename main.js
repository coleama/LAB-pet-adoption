...
/*const targetingApp = document.querySelector("#PetCard");
  let domString = ""; {
...
showCatButton.addEventListener("click", () => {
  const catMemeber = filter(pet, 'cat');
  cardsOnDom(catMemeber);
...
showDogButton.addEventListener('click',() => {
  const dogButton = filter(pet, 'dog');
  cardsOnDom(dogButton)
});
showDinoButton.addEventListener('click', () => {
  const dinoButton = filter(pet, 'dino');
...
submitButton.addEventListener('click', newPet);

const targetingPetCard = document.querySelector("#PetCard");

targetingPetCard.addEventListener("click", (event) =>{
  if(event.target.id.includes("delete")){
    const [throwAway, petId] = event.target.id.split("--");
    const index = pet.findIndex((object) => object.id === Number(petId));
    pet.splice(index, 1);
  }
  cardsOnDom(pet);
});
